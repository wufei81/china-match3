...
**Lead:** Coordinator who synthesizes findings and facilitates debate **Teammates:** 2-3 investigators, each assigned a
...
```markdown
I need to debug a complex issue with multiple plausible causes: [DESCRIBE ISSUE - e.g., "API latency spiked from 200ms
to 5s starting at 14:00 UTC"]
...
New search algorithm generates complex JOIN with 4 tables. Query plan shows full table scan on `search_terms` table (2M
rows).
...
New search ranking algorithm contains O(n²) nested loop that sorts results twice. With 500-1000 results per search, this
causes 4+ second CPU-bound processing time.
...
**Sequential investigation:** ~60-90 minutes (20-30 min per hypothesis) **Parallel investigation:** ~25-35 minutes (all
...
**Team:** 1 Opus lead + 3 Sonnet investigators **Duration:** ~30 minutes **Estimated cost:** $18-25
