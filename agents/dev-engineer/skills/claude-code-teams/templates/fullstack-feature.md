...
**Lead:** Architect who defines contracts and coordinates integration **Teammates:** 3 specialists
...
**Integration checkpoint:** After all teammates finish:
...
- Backend engineer: Only edit files under `backend/src/controllers/profile.ts`, `backend/src/services/profile.ts`,
...
Authorization: Bearer {jwt_token} Content-Type: multipart/form-data (if photo included) or application/json
...
**Sequential implementation:** ~90-120 minutes (30-40 min per layer) **Parallel implementation:** ~35-45 minutes (all
...
**Team:** 1 Opus lead + 3 Sonnet specialists **Duration:** ~40 minutes **Estimated cost:** $20-30
...
**Symptom:** Frontend sends `userName` but backend expects `name` **Prevention:** Define contract first, both sides
implement exactly as specified **Fix:** Update implementation to match contract (don't change contract mid-stream)

### Issue: Merge conflicts

**Symptom:** Git conflicts when merging branches **Prevention:** Strict file boundaries (frontend/backend/tests don't
overlap) **Fix:** Review boundaries, ensure each teammate owns distinct files

### Issue: Tests fail due to missing setup

**Symptom:** Integration tests fail because database or S3 mock not configured **Prevention:** Test engineer documents
required setup in test README **Fix:** Add setup script or fixture data
