...
def cmd_add(args):
    ...
    memory_id = memory.add_memory(
        text=args.text, category=args.category, metadata={"tags": args.tags.split(",") if args.tags else []}
    )
    print(f"✅ 记忆已添加")
...
def cmd_search(args):
    """搜索记忆"""
    memory = VectorMemory()
    results = memory.search_memories(query=args.query, n_results=args.limit, category=args.category)
